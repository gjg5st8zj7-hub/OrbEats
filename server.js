...
const VIRUS_POP = 130;
const VIRUS_FEEDS = 7;
const VIRUS_LOOKS = ['plasma','storm','ember','pulse'];
const POWER_N = { ice: 4, resin: 4, triad: 8 };
const MAX_CELLS = 16;
...
const SKINS = ['chrome','glass','mangrove','surge','manatee','dawn','rust','copper','drydock','flame','bilge','weld','tarpon','cuda','shark','lionfish','gator','osprey','blackwater','nebula','comet','eclipse','phosphor','venom','jaw','lava','hotsauce','cotton','king','ghost','pearl','trophy','ice','coral','plasma','honey','midnight','magma','prism','pacdan','koi','blep','aurora','snooze','honeycomb','lotus','drip','thunder','abalone','woah','emberglass','satellite','mosaic','sus','nerd','boo','kiss','rawr','googly','derp'];
...
function makeVirus(p) {
  p = p || spawnPos();
  return { x: p.x, y: p.y, mass: 100, r: massToR(100), spikes: 14, feeds: 0, flying: false, vx: 0, vy: 0, look: pick(VIRUS_LOOKS) };
}
...
function spawnBot() {
  ...
  const skill = rand(0.32, 0.88);
  const roll = Math.random();
  const mass = roll < 0.62 ? rand(18, 26) : roll < 0.88 ? rand(40, 95) : rand(110, 220);
...
function botThink(p, dt) {
  ...
  if (ai.moodT <= 0) {
    const r = Math.random();
    ai.mood = r < 0.22 ? 'farm' : r < 0.62 ? 'hunt' : r < 0.78 ? 'bully' : r < 0.90 ? 'roam' : 'hide';
...
function moveCells(p, dt) {
  ...
  for (const c of p.cells) {
    ...
    const iced = !!(p.iceUntil && now < p.iceUntil && totalMass(p) <= 5000);
    const spd = iced ? speedOf(Math.min(c.mass, 90)) * 2.15 : speedOf(c.mass);
    ...
    if (p.resinCell === c && p.resinUntil && now < p.resinUntil && p.resinFrom != null && p.resinTo != null) {
      const tR = 1 - Math.max(0, p.resinUntil - now) / 5000;
    ...
    } else if (p.resinCell === c && p.resinUntil && now >= p.resinUntil && p.resinTo != null) {
  ...
  for (const p of players) {
    ...
    for (const c of p.cells) {
      ...
      for (let i = powers.length - 1; i >= 0; i--) {
        const u = powers[i];
        if (Math.hypot(u.x - c.x, u.y - c.y) < c.r + u.r) {
          const ts = Date.now();
          if (u.kind === 'triad') { c.triad = true; c.triadAt = ts + 800; }
          else if (u.kind === 'ice') {
            if (totalMass(p) <= 5000) { p.iceUntil = ts + 10000; p.resinUntil = 0; }
          } else if (u.kind === 'resin') {
            const add = 50 + Math.random() * 450;
            p.resinCell = c; p.resinFrom = c.mass; p.resinTo = c.mass + add;
            p.resinUntil = ts + 5000; p.iceUntil = 0;
          }
          powers.splice(i, 1);
          break;
        }
        ...
        for (const o of players) {
          ...
          for (const oc of o.cells) {
            if (Math.hypot(oc.x - c.x, oc.y - c.y) < reach + oc.r * 0.45) {
              if (o.cells.length < MAX_CELLS && oc.mass >= SPLIT_MIN) {
                const now2 = Date.now();
                const ang2 = Math.random() * Math.PI * 2;
                const half = oc.mass / 2;
                oc.mass = half; oc.r = massToR(half);
                oc.mergeAt = now2 + mergeMs(half);
                const child = makeCell(oc.x + Math.cos(ang2) * 10, oc.y + Math.sin(ang2) * 10, half, { mergeAt: now2 + mergeMs(half), ghostUntil: now2 + GHOST_MS });
                child.bx = Math.cos(ang2) * SPLIT_BOOST * 0.7;
                child.by = Math.sin(ang2) * SPLIT_BOOST * 0.7;
                o.cells.push(child);
              }
              c.triad = false; c.triadAt = 0;
              break;
            }
          }
          if (!c.triad) break;
...
function snapshotFor(p) {
  ...
  const pw = powers.filter(u => Math.abs(u.x - me.x) < view && Math.abs(u.y - me.y) < view).map(u => ({ x: u.x, y: u.y, r: u.r, kind: u.kind }));
...
setInterval(() => {
  ...
  for (const p of players) {
    ...
    if (!p.bot && p.alive && (tickN % 15 === 0)) recordScore(p);
  }
  eatPass();
  for (const p of players) {
    if (!p.sock) continue;
    const split = p.alive && p.cells && p.cells.length >= 8;
    if (split && (tickN % 2)) continue; // 15hz packets when 8+ pieces
