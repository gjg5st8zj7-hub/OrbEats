...
function spawnBot() {
  ...
  const botSkins = SKINS.filter(s => s !== 'pacdan');
  return makePlayer({
...
function fillLobby() {
  const now = Date.now();
  players = players.filter(p => p.alive || p.sock || (p.resumeUntil && now < p.resumeUntil));
...
function moveCells(p, dt) {
  ...
  for (const c of p.cells) {
    ...
    let spd = iced ? speedOf(Math.min(c.mass, 90)) * 2.15 : speedOf(c.mass);
...
function snapshotFor(p) {
  ...
  const view = Math.min(2200, 1100 + Math.sqrt(me.m) * 14);
  const plist = [];
  for (const o of players) {
    if (!o.alive) continue;
    const c = centroid(o);
...
setInterval(() => {
  ...
  for (const p of players) {
    ...
    if (!p.bot && p.alive && (tickN % 20 === 0)) recordScore(p);
  }
  eatPass();
  for (const p of players) {
    if (!p.sock) continue;
...
io.on('connection', sock => {
  ...
  sock.on('join', data => {
    ...
    const now = Date.now();
    const ghost = players.find(p => !p.bot && p.cells && p.cells.length
      && (p.alive || (p.resumeUntil && now < p.resumeUntil))
      && ((pid && p.pid && p.pid === pid) || (!pid && p.name.toLowerCase() === name.toLowerCase())));
    if (ghost) {
      if (ghost.sock && ghost.sock !== sock) {
        try { ghost.sock.emit('taken'); ghost.sock.disconnect(true); } catch (_) {}
      }
      for (const q of players) {
        if (q !== ghost && !q.bot && (q.sock === sock || (pid && q.pid === pid))) {
          q.alive = false; q.sock = null; q.resumeUntil = 0; q.cells = [];
        }
      }
      ghost.sock = sock;
      ghost.alive = true;
      ghost.pid = pid || ghost.pid;
      ghost.name = name || ghost.name;
      ghost.resumeUntil = now + 60000;
      ghost.protectUntil = Math.max(ghost.protectUntil || 0, now + 2000);
    ...
    for (const q of players) {
      if (q.bot) continue;
      if (q.sock === sock || (pid && q.pid === pid)) {
        q.alive = false; q.sock = null; q.resumeUntil = 0; q.cells = [];
      }
    }
    for (const q of players) {
      if (q.sock === sock) { q.alive = false; q.sock = null; }
    }
    const p = makePlayer({
      sock, name, pid,
      skin: (data && data.skin) || '',
