...
setInterval(() => {
  ...
  for (const p of players) {
    if (!p.sock) continue;
    const n = p.alive && p.cells ? p.cells.length : 0;
    if (n >= 12 && (tickN % 3)) continue;
    else if (n >= 8 && (tickN % 2)) continue;
...
io.on('connection', sock => {
  ...
  sock.on('join', data => {
    ...
    const now = Date.now();
    const mine = players.find(p => p.sock === sock);
    const ghost = players.find(p => !p.bot && p.alive && p.cells && p.cells.length
      && p.name.toLowerCase() === name.toLowerCase()
      && p.sock !== sock
      && p.resumeUntil && now < p.resumeUntil);
    if (ghost) {
      if (mine && mine !== ghost) { mine.alive = false; mine.sock = null; }
      ghost.sock = sock;
      ghost.resumeUntil = now + 60000;
      ghost.protectUntil = now + 2500;
      if (data && data.skin) ghost.skin = data.skin;
      sock.emit('joined', { id: ghost.id, resume: true });
      return;
    }
    if (mine) { mine.alive = false; mine.sock = null; }
    const p = makePlayer({
      sock, name,
      skin: (data && data.skin) || '',
      mass: 22,
