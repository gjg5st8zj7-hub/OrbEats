...
const io = new Server(server, { cors: { origin: '*' }, pingInterval: 8000, pingTimeout: 20000, transports: ['websocket', 'polling'] });
...
const SKINS = ['chrome','squint','mangrove','blep','manatee','snooze','rust','copper','drip','flame','bilge','woah','tarpon','cuda','shark','lionfish','gator','osprey','blackwater','nebula','comet','eclipse','phosphor','venom','jaw','lava','hotsauce','sus','king','ghost','pearl','nerd','boo','kiss','plasma','rawr','midnight','magma','googly','derp'];
...
setInterval(() => {
  ...
  if (tickN % 5 === 0) fillLobby();
    const nowR = Date.now();
    for (const p of players) {
      if (!p.bot && p.alive && !p.sock && p.resumeUntil && nowR > p.resumeUntil) {
        p.alive = false; p.resumeUntil = 0;
      }
    }
  for (const p of players) {
    if (!p.alive) continue;
...
io.on('connection', sock => {
  ...
  sock.on('join', data => {
    const already = players.find(p => p.sock === sock && p.alive);
    if (already) { sock.emit('joined', { id: already.id }); return; }
    const old = players.find(p => p.sock === sock);
    if (old) { old.alive = false; old.sock = null; }
    const nm = ((data && data.name) || 'Unnamed').slice(0, 16);
    const resume = players.find(x => !x.bot && x.alive && !x.sock && x.name === nm && x.resumeUntil && Date.now() < x.resumeUntil);
    if (resume) {
      resume.sock = sock;
      resume.resumeUntil = 0;
      sock.emit('joined', { id: resume.id });
      return;
    }
    const p = makePlayer({
      sock, name: nm,
  ...
  sock.on('disconnect', () => {
    const p = players.find(x => x.sock === sock);
    if (p && !p.bot) { p.sock = null; p.resumeUntil = Date.now() + 60000; }
