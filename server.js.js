...
function snapshotFor(p) {
  ...
  const view = Math.min(2400, 1100 + Math.sqrt(me.m) * 12);
  const plist = [];
  for (const o of players) {
    if (!o.alive) continue;
    const c = centroid(o);
  ...
  const stride = nearFood.length > 160 ? Math.ceil(nearFood.length / 160) : 1;
  const fd = nearFood.filter((_, i) => i % stride === 0 || nearFood[i].pellet).slice(0, 180).map(f => ({ x: f.x, y: f.y, r: f.r, color: f.color }));
