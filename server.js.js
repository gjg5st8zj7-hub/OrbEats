...
function snapshotFor(p) {
  ...
  const stride = nearFood.length > 280 ? Math.ceil(nearFood.length / 280) : 1;
  const fd = nearFood.filter((_, i) => i % stride === 0 || nearFood[i].pellet).slice(0, 320).map(f => ({ x: f.x, y: f.y, r: f.r, color: f.color }));
